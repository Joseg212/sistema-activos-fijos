...
class json_lstReparacion
{
	...
	private function queryFilter()
	{
		...
		if (empty($txtSearch)){
		    	$sql = "
                    ...
                    WHERE mant.tipo_mant = 'Reparacion' AND mant.id_af = '{$idActivo}'
		    	...
		    	$sql = " 
                    ...
                    FROM App\Entity\Mantenimiento mant
                    INNER JOIN App\Entity\Activofijo act WITH mant.id_af=act.id_af AND act.estatus='activo'
                    WHERE mant.tipo_mant = 'Reparacion' AND mant.id_af = '{$idActivo}' AND {$condiBusca}
    ...
    public function responseData($docTrine)
    {
        ...
        switch ($this->arrOrder[0]['column']) {
            ...
            case 4:
                $this->fieldOrder = 'mant.proveedor';
                break;

            case 5:
                $this->fieldOrder = 'mant.numero_tiempo';
                break;
        ...
        {
            $data['data'][]=array(
                'id'=>'','activo'=>'','fecha'=>'','detalle'=>'','proveedor'=>'','tiempo'=>'','total'=>'');
        }
