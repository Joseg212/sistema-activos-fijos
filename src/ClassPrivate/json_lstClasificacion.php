...
class json_lstClasificacion
{
	...
	private function queryFilter()
	{
		...
		if (empty($txtSearch)){
		    	$sql = "
                    SELECT p.id_clase,p.descripcion,p.cod_cuenta,p.observacion
                    FROM App\Entity\Clasificacion p
    ...
    public function responseData($docTrine)
    {
        ...
        $this->funcGlobal->setarrFieldSearch(array("p.id_clase,p.descripcion,p.cod_cuenta,p.observacion"));
        ...
        switch ($this->arrOrder[0]['column']) {
            case 0:
                ...
                break;
            case 1:
                $this->fieldOrder = 'p.descripcion';
                break;
            case 2:
                $this->fieldOrder = 'p.cod_cuenta';
                break;
            case 3:
                $this->fieldOrder = 'p.observacion';
                break;
        }
	    ...
	    foreach ($resultArray as $fila) {
            $html_col1 = '<p>'.$fila['id_clase'].'</p><input type="hidden" id="name_'.$fila['id_clase'].'" value="'.$fila['descripcion'].'" />';
            $nohay=false;
	    	$data['data'][]=array(
                "DT_RowId"=> $fila['id_clase'],
