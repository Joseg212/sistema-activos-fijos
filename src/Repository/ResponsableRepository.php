...
class ResponsableRepository extends ServiceEntityRepository
{
    ...
    */
    public function findOneByDataResp($idResp): array
    {
        $data = array();

        $em = $this->getEntityManager();

        $sql = "SELECT u FROM App\Entity\Responsable u WHERE u.id_resp = '{$idResp}'";

        $qry_row = $em->createQuery($sql);

        if ($qry_row)
        {
            $rst = $qry_row->getSingleResult();
            $data = [
                'nombre' => $rst->getNombre(). ' '. $rst->getApellido(),
                'cargo' => $rst->getCargo(),
                'telefono' => $rst->getTelefono(),
                'movil' => $rst->getMovil(),
            ];
        } else {
            $data = [
